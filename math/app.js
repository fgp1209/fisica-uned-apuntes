(() => {
  ...
  function load() {
    ...
    lines.forEach(line => createLine(normalizeLatex(line)));
    setTimeout(() => focusField(paper.querySelector('math-field')), 200);
  }

  document.querySelectorAll('[data-latex]').forEach(btn => {
    btn.addEventListener('click', () => insertLatex(normalizeLatex(btn.dataset.latex)));
  });
  ...
  toggleMathKeyboardBtn.addEventListener('click', () => {
    ...
    if (typeof activeField.executeCommand === 'function') {
      try { activeField.executeCommand(['toggleVirtualKeyboard']); }
      catch { activeField.executeCommand('toggleVirtualKeyboard'); }
    }
  });
